<script lang="ts">
	...
	import { isLoggedIn, userProfile, identity as authIdentity, fetchUserProfile } from '$lib/stores/authStore';
	...
	const handleRegistration = async () => {
		...
		try {
			...
			registrationError = error.message?.includes("User already registered") ? "Already registered. Refreshing..." : 'Registration failed.';
...
<div class="container mx-auto p-4 min-h-screen">
	{#if showRegistration}
		<div class="registration-form bg-white p-8 rounded-lg shadow-md max-w-md mx-auto my-10">
			...
			<input type="text" bind:value={registrationName} placeholder="Your Name" class="w-full px-3 py-2 border rounded mb-4" disabled={registrationLoading}/>
			<button on:click={handleRegistration} disabled={registrationLoading} class="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded disabled:opacity-75">
		...
		<div class="calendar-management mb-6 p-4 bg-white rounded-lg shadow">
			<div class="flex flex-col sm:flex-row items-center justify-between gap-4">
				<div class="flex-grow w-full sm:w-auto">
					<label for="calendar-select" class="block text-sm font-medium text-gray-700 mb-1">Select Calendar:</label>
					...
					{:else if userCalendars.length > 0}
						...
						>
							{#each userCalendars as cal (cal.id)}
								<option value={cal.id.toString()}>{cal.name}</option>
							{/each}
						</select>
					{:else}
			...
			{#if showNewCalendarForm}
				<div class="new-calendar-form mt-4 pt-4 border-t">
					...
					<input type="text" bind:value={newCalendarName} placeholder="Calendar Name" class="w-full px-3 py-2 border rounded mb-2" />
					<div class="flex items-center mb-3">
						<label for="new-calendar-color" class="mr-2 text-sm">Color:</label>
						<input type="color" id="new-calendar-color" bind:value={newCalendarColor} class="h-8 w-12 border rounded" />
					</div>
					<button on:click={handleCreateNewCalendar} disabled={newCalendarLoading} class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded text-sm disabled:opacity-75">
